  ...
  return {
    ...
    ...(extended ? (() => {
      return {
        ...
        richBadges: Array.isArray(extended.badges) ? extended.badges as { badge_type: string; label?: string; description?: string; project_name?: string; awarded_at: string }[] : undefined,
      }
    })() : {}),
...
export async function getProfileBySlug(slug: string): Promise<Profile | null> {
  ...
  try {
    // Try published first, then any visibility
    ...
    if (published) {
      ...
      // Fallback: any visibility (for admin/preview access)
      const { data: any } = await supabaseAdmin
        .from('user_profiles')
        .select('*')
        .eq('slug', slug)
        .single()
      if (any) match = any as Record<string, unknown>
    }

    // Legacy fallback: match by slugified display_name for profiles without slug column
    if (!match) {
